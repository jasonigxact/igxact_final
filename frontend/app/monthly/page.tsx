          ...
          <section className="section">
            ...
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:24, alignItems:"center" }}>
              <ResponsiveContainer width="100%" height={300}>
                <PieChart>
                  <Pie data={data.revenue_breakdown} dataKey="value" nameKey="name" outerRadius={95} innerRadius={48} paddingAngle={3} label={(e: any) => `${(e.percent*100).toFixed(1)}%`}>
                    {data.revenue_breakdown.map((_: any, i: number) => <Cell key={i} fill={EXPENSE_COLORS[i % EXPENSE_COLORS.length]} />)}
                  </Pie>
                  <Tooltip contentStyle={tooltipStyle} formatter={(v: any) => `₹${Number(v).toLocaleString("en-IN")}`} />
              ...
              <div style={{ display:"flex", flexDirection:"column", gap:6 }}>
                {data.revenue_breakdown.map((e: any, i: number) => {
                  const total = data.revenue_breakdown.reduce((s: number, r: any) => s + (Number(r.value)||0), 0);
                  const pct = total > 0 ? ((Number(e.value)/total)*100).toFixed(1) : "0";
                  const color = EXPENSE_COLORS[i % EXPENSE_COLORS.length];
                  return (
                    <div key={i} style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"8px 12px", background:"rgba(0,0,0,0.02)", borderRadius:8, border:`1px solid ${color}25` }}>
                      <div style={{ display:"flex", alignItems:"center", gap:8 }}>
                        <div style={{ width:10, height:10, borderRadius:"50%", background:color, flexShrink:0 }} />
                        <span style={{ fontSize:13, fontWeight:600 }}>{e.name}</span>
                      </div>
                      <div style={{ display:"flex", gap:10, alignItems:"center" }}>
                        <span style={{ fontSize:13, fontWeight:700 }}>₹{Number(e.value).toLocaleString("en-IN")}</span>
                        <span style={{ fontSize:11, fontWeight:700, background:`${color}18`, color, padding:"2px 8px", borderRadius:6, minWidth:44, textAlign:"center" }}>{pct}%</span>
                      </div>
                    </div>
                  );
                })}
